...
class DSparkVariant(Enum):
    ...
    The value is the draft architecture vLLM loads. Qwen3, Qwen3-Omni, Gemma4
    and K3 declare theirs in the checkpoint; DeepSeek-V4 DSpark reuses the
    target's own DeepSeek-V4 config and has no draft architecture of its own,
    so vLLM synthesises `DSparkDraftModel` for it in `_verify_args`.
    """

    QWEN3 = "Qwen3DSparkModel"
    QWEN3_OMNI = _QWEN3_OMNI_DSPARK_ARCHITECTURE
            ...
            if self.model is not None:
                ...
                if self.method == "dspark":
                    ...
                    if dspark_draft_topk is not None:
                        ...
                        if dspark_variant not in (
                            DSparkVariant.QWEN3,
                            DSparkVariant.QWEN3_OMNI,
                        ):
                            raise ValueError(
